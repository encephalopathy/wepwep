...
local function removeStateLayers ( state, stackLoc )
...
----------------------------------------------------------------
function push ( screenState, ... ) --stateFile is replaced with screenState
	...
	--local newState = loadState ( screenState )
	--loadedState[screenState] = screenState
	table.insert ( stateStack, screenState )	
	curState = stateStack [ #stateStack ]
	...
	if curState.IS_POPUP then
		...
		removeStateLayers( curState, #stateStack )
		rebuildRenderStack ()
	end
end
